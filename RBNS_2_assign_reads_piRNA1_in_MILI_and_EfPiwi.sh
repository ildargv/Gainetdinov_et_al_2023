...
for(k=6;k<=11;k++){for (g=2;g<=(27-k);g++){num=split( full, temp, substr(\"TGGATACTATACAACCTACTACCTC\",(28-k)-g,k)); if(num>2){sites=2}; if((num==2)&&(substr(temp[1],length(temp[1]),1)!=substr(\"NTGGATACTATACAACCTACTACCTCN\",(28-k)-g,1))&&(substr(temp[2],1,1)!=substr(\"NTGGATACTATACAACCTACTACCTCN\",29-g,1)))
...
for f in $(ls *.trimmed.xx.????); do /project/umw_phil_zamore/common/pipelines/ms_1G_1core "awk 'BEGIN{FS=OFS=\"\\t\"}(FNR%4==2){if((length(\$1)==20)&&(index(\$1,\"N\")==0)){sites=0;site_loc=\"\";full=\"GATC\"\$1\"TGGA\";
for(k=6;k<=10;k++){num=split( full, temp, substr(\"TGGATACTATACAACCTACTACCTC\",(28-k)-2,k)); if(num>2){sites=2}; if((num==2)&&(substr(temp[1],length(temp[1]),1)!=substr(\"NTGGATACTATACAACCTACTACCTCN\",(28-k)-2,1)))
